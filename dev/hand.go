...
func streetLoop(h *Hand) {
	...
	if h.currentState == "pre-flop" {
		h.avaliableActions = []string{"big_blind", "small_blind"}
		print("small blind is: ", h.Players[h.actionPlayerIndex].ID, "\n")
		handleAction(h, Action{PlayerID: h.Players[h.actionPlayerIndex].ID, Action: "small_blind", Amount: h.smallBlindSize})
		//big blind raise
		h.actionPlayerIndex = (h.actionPlayerIndex + 1) % len(h.Players)
		print("big blind is: ", h.Players[h.actionPlayerIndex].ID, "\n")
		handleAction(h, Action{PlayerID: h.Players[h.actionPlayerIndex].ID, Action: "big_blind", Amount: h.smallBlindSize})
		h.avaliableActions = []string{"raise", "fold", "clear", "call"}
